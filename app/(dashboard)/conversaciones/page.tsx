...
import { ConversationsTable } from "@/components/conversaciones/conversations-table";
import { Button } from "@/components/ui/button";
import { useRefreshContext } from "@/store/refresh-context";
import { getDateFilter, type DatePreset } from "@/lib/date-filters";
import { buildConversationRows } from "@/lib/conversations-aggregation";
...
export default function ConversacionesPage() {
  ...
  const rows = useMemo(
    () => buildConversationRows(chats, agents, metricsItems),
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            size="sm"
            onClick={() =>
              exportToCsv(rows, "conversaciones", [
                { key: "chatId", label: "ID" },
                { key: "customerName", label: "Cliente" },
                { key: "agentName", label: "Agente" },
                { key: "investmentReason", label: "Motivo" },
                { key: "adSource", label: "Pauta" },
                { key: "closingTypification", label: "Tipificación" },
                { key: "firstResponseTime", label: "1ra Respuesta" },
                { key: "avgChatDuration", label: "Duración" },
                { key: "conversationLink", label: "Link Conversación" },
              ])
            }
